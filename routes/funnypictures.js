...
router.get('/categorie/:categorie',function(req,res,next){
	...
	res.redirect("/funnypictures/categorie/"+categorie+"/1");
});
...
router.get('/categorie/:categorie/:page([0-9]+)', function(req, res, next) {
	...
	var query = "select * from pictures where categorie like '"+categorie+"' order by dateAdd DESC  limit "+nbrPost+" offset "+offset;
	var query1 = "select distinct(categorie) from pictures";
	var query2 = "select count(*) as nbrPost from pictures where categorie like '"+categorie+"'";
	//var query2 = "select distinct(categorie) from games";
	var query = query+";"+query1+";"+query2;

	pagination(res,query,page,offset,"picturesCategorie.html.twig",categorie,next);
...
router.get('/funnypicture/:id',function(req,res,next){
	...
	pool.getConnection(function(err,connection){
		...
		connection.query(query,function(err,categories){
			connection.release();
			//if(err) throw err;
			var query = "select * from pictures where filename like ?";

			post(req,res,query,"pictures",categories,filename,next);
...
router.get("/like/",function(req,res,next){
	var id = (req.query.id);
	var query = "update pictures set likes = likes+1 where id = "+(id);
	pool.getConnection(function(err,connection){
...
function post(req,res,query,table,categories,filename,nextt){
	...
	pool.getConnection(function(err,connection){
		...
		var picture = connection.query(query,[filename],function(err,results){
			...
			var query7 = connection.query(query,[id,id],function(err,result,fields){
				connection.release();
				...
				if(result.length == 1){
					...
					parametres = {
							"post":results,
							"next":(result[0].filename).split(".")[0],
							"previous":(result[1].filename).split(".")[0]
						};
...
router.get('/page/:page([0-9]+)', function(req, res, next) {
	...
	var query = "select * from pictures order by timeAdd desc  limit "+nbrPost+" offset "+pool.escape(offset)+" ";
	var query1 = "select distinct(categorie) from pictures";
